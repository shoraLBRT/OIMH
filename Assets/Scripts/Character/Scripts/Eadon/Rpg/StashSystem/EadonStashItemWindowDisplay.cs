...
{
    public class EadonStashItemWindowDisplay : MonoBehaviour
    {
        public vInventory inventory;
        public vItemWindow itemWindow;
        public EadonStashItemOptionWindow optionWindow;
        public EadonStashTransactionManager transactionManager;
        [HideInInspector]
        ...
        public virtual void OnEnable()
        {
            if (inventory == null)
            {
                var stash = EadonStashManager.Instance.currentStash;
                if (stash)
                {
        ...
        public void TakeItem()
        {
            ...
            {
                transactionManager.TakeItem(currentSelectedSlot.item, amount);
            }
        }

        public virtual void StashItem()
        {
            if (amount <= 0) return;
            if (transactionManager != null)
            {
                transactionManager.StashItem(currentSelectedSlot.item, amount);
            }
        }
