...
{
    [vClassHeader("Clothing Equip Area", openClose = false)]
    public class ClothingEquipArea : vMonoBehaviour
    {
        public delegate void OnPickUpItem(vEquipArea area, vItemSlot slot);
        public OnPickUpItem onPickUpItemCallBack;

        public vInventory inventory;
        public ClothingItemWindow itemPicker;
        [Tooltip("Set current equiped slot when submit an slot of this area")]
        public bool setEquipSlotWhenSubmit;
        public List<EadonClothingEquipSlot> equipSlots;
        public EadonClothingSetEquipSlot clothingSetSlot;
        ...
        public OnSelectClothingEquipArea onSelectClothingEquipArea;
        [HideInInspector]
        public EadonClothingEquipSlot currentSelectedSlot;
        public EadonClothingEquipSlot lastSelectedSlot;
        [HideInInspector]
        public int indexOfEquippedItem;
        public vItem lastEquipedItem;

        private EadonCharacterAppearance _characterAppearance;
        private ClothingManager _clothingManager;
        private bool _selectingSet;
        ...
        void Start()
        {
            if (!isInit)
            {
                ...
                _characterAppearance = GetComponentInParent<EadonCharacterAppearance>();
                var character = GetComponentInParent<EadonRpgCharacterBase>();
                if (character != null)
                {
                ...
                {
                    var equipSlotsArray = GetComponentsInChildren<EadonClothingEquipSlot>(true);
                ...
                if (clothingSetSlot != null)
                {
                    clothingSetSlot.onSubmitSlotCallBack = OnSubmitSlot;
                    clothingSetSlot.onSelectSlotCallBack = OnSelectSlot;
                    clothingSetSlot.onDeselectSlotCallBack = OnDeselect;
                }
            }
        }
        ...
        /// </summary>
        public List<EadonClothingEquipSlot> ValidSlots
        {
        ...
        public void OnSubmitSlot(vItemSlot slot)
        {
            ...
            if (itemPicker != null)
            {
                if (slot is EadonClothingEquipSlot)
                {
                    _selectingSet = false;
                    currentSelectedSlot = slot as EadonClothingEquipSlot;
                    ...
                    itemPicker.CreateEquipmentWindow(inventory.items, currentSelectedSlot.itemType, currentSelectedSlot.clothingHolder, slot.item, OnPickItem);
                    onInitPickUpItem.Invoke();
                }
                else if (slot == clothingSetSlot)
                {
                    _selectingSet = true;
                    ...
                    itemPicker.onCancelSlot.RemoveAllListeners();
                    itemPicker.onCancelSlot.AddListener(CancelCurrentSlot);
                    itemPicker.CreateEquipmentWindow(inventory.items, clothingSetSlot.itemType, -1, slot.item, OnPickItem);
                    onInitPickUpItem.Invoke();
                }
            }
        }
...
//                onUnequipItem.Invoke(this, item);
        ...
        public void UnequipCurrentItem()
        {
            if (currentSelectedSlot && currentSelectedSlot.item)
            {
                ...
                if (_characterAppearance != null)
                {
                    if (_clothingManager != null)
                    {
                        var item = _item;
                        var clothingIdAttribute = item.GetItemAttribute(vItemAttributes.ClothingID);
                        if (clothingIdAttribute != null)
                        {
                            var clothing = _clothingManager.GetItemInstance(clothingIdAttribute.value);
                            if (clothing != null)
                            {
                                _characterAppearance.RemoveClothing(clothing);
                            }
                            foreach (var blockedHolderName in clothing.blockedHolders)
                            {
                                foreach (var equipSlot in equipSlots)
                                {
                                    if (equipSlot.clothingHolder == (int) blockedHolderName)
                                    {
                                        equipSlot.SetValid(true);
                                        if (clothing.removeDefaultWhenBlocked)
                                        {
                                            _characterAppearance.GetClothingHolder(blockedHolderName).SetDefaultClothingVisibility(true);
                                        }
                                    }
                                }
                            }
        ...
        {
            if (equipSlots.Contains(slot as EadonClothingEquipSlot))
                currentSelectedSlot = slot as EadonClothingEquipSlot;
            else currentSelectedSlot = null;

            onSelectClothingEquipArea.Invoke(this);
        ...
        public void OnPickItem(vItemSlot slot)
        {
            if (_selectingSet)
            {
                var clothingSetIdAttribute = slot.item.GetItemAttribute(vItemAttributes.ClothingID);
                if (clothingSetIdAttribute != null)
                {
                    var clothingSet = _clothingManager.GetItemSetInstance(clothingSetIdAttribute.value);
                    if (clothingSet != null)
                    {
                        if (_characterAppearance.currentClothingSet != null && _characterAppearance.removeAllPreviousWhenChangingSet)
                        {
                            // remove old
                            foreach (var clothingItem in _characterAppearance.currentClothingSet.allItems)
                            {
                                foreach (var blockedHolderName in clothingItem.blockedHolders)
                                {
                                    foreach (var equipSlot in equipSlots)
                                    {
                                        if (equipSlot.clothingHolder == (int) blockedHolderName)
                                        {
                                            equipSlot.SetValid(true);
                                            if (clothingItem.removeDefaultWhenBlocked)
                                            {
                                                _characterAppearance.GetClothingHolder(blockedHolderName).SetDefaultClothingVisibility(true);
                                            }
                                        }
                                ...
                                var item = _clothingManager.GetItemInstance(clothingId);
                                _characterAppearance.RemoveClothing(item);
                            }
                        }
                        foreach (var clothingItem in clothingSet.allItems)
                        {
                            var clothingId = clothingItem.clothingId;
                            var items = inventory.items.FindAll(i => i.type == vItemType.Clothing);
                            foreach (var item in items)
                            {
                                if (item.GetItemAttribute(vItemAttributes.ClothingID).value == clothingId)
                                {
                                    if (_characterAppearance != null)
                                    {
                                        if (_clothingManager != null)
                                        {
                                            WearClothingFromItem(item);
                                        }
                                    }
                                    foreach (var equipSlot in equipSlots)
                                    {
                                        if (equipSlot.clothingHolder == (int) clothingItem.clothingHolderName)
                                        {
                            ...
                            foreach (var blockedHolderName in clothingItem.blockedHolders)
                            {
                                foreach (var equipSlot in equipSlots)
                                {
                                    if (equipSlot.clothingHolder == (int) blockedHolderName)
                                    {
                                        equipSlot.SetValid(false);
                                        if (clothingItem.removeDefaultWhenBlocked)
                                        {
                                            _characterAppearance.GetClothingHolder(blockedHolderName).SetDefaultClothingVisibility(false);
                                        }
                                    }
                                }
                            }
                        ...
                        _characterAppearance.currentClothingSet = clothingSet;
                    }
                }
            }
            else
...
//                onEquipItem.Invoke(this, currentSelectedSlot.item);
        ...
        private void WearClothingFromItem(vItem item)
        {
            var clothingIdAttribute = item.GetItemAttribute(vItemAttributes.ClothingID);
            if (clothingIdAttribute != null)
            {
                var clothing = _clothingManager.GetItemInstance(clothingIdAttribute.value);
                var currentClothingItem = _characterAppearance.GetClothingHolder(clothing.clothingHolderName)
                    .GetCurrentClothingItem();
                if (currentClothingItem != null)
                {
                    foreach (var blockedHolderName in currentClothingItem.blockedHolders)
                    {
                        foreach (var equipSlot in equipSlots)
                        {
                            if (equipSlot.clothingHolder == (int) blockedHolderName)
                ...
                if (clothing != null)
                {
                    _characterAppearance.WearClothing(clothing);
                }
            }
        }
        ...
        {
            if (slot is vEquipSlot && equipSlots.Contains(slot as EadonClothingEquipSlot))
            {
                AddItemToEquipSlot(equipSlots.IndexOf(slot as EadonClothingEquipSlot), item, autoEquip);
            }
        }
...
//                        onEquipItem.Invoke(this, item);
        ...
        public void RemoveItemOfEquipSlot(vItemSlot slot)
        {
            if (slot is vEquipSlot && equipSlots.Contains(slot as EadonClothingEquipSlot))
            {
                RemoveItemOfEquipSlot(equipSlots.IndexOf(slot as EadonClothingEquipSlot));
            }
        }
